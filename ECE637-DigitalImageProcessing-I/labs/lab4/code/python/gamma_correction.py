...
def gamma_correct_from_linear(img,gamma_out):
    ...
    for i in range(h):
        for j in range(w):
            #inverse of eq (5)    
            out_img[i,j] = 255.0*np.exp((1.0/gamma_out)*np.log(img[i,j]/255.0))
    return out_img
...
def gamma_correct_from_gamma(img,gamma_out,gamma_in):
    ...
    for i in range(h):
        for j in range(w):
            x = 255.0*(img[i,j]/255.0)**gamma_in
            out_img[i,j] = 255.0*np.exp((1.0/gamma_out)*np.log(x/255.0))    
    return out_img
...
if __name__=="__main__":
    ...
    plt.savefig(basename+'_gamma_corrected.eps',format='eps')
    plt.close()
