function [X,Y,Z,spring_initial_radius,spring_max_radius]=read_profile(parent_dir,N,wire_dia)
...
f2=figure(2);
plot(height,diameter,'-r',Z/inchestomm,splinedia);%not the centerline dia
xlabel('height(in)');ylabel('diameter(in)');set(findall(f2, 'Type', 'Line'),'LineWidth',4);set(gca,'FontSize',15);
saveas(f2,strcat(parent_dir,'/height_vs_diameter.png'));

f3=figure(3);
plot(angle,diameter,'-r',theta,splinedia);%not the centerline dia
xlabel('theta(rad)');ylabel('diameter(in)');set(findall(f3, 'Type', 'Line'),'LineWidth',4);set(gca,'FontSize',15);
saveas(f3,strcat(parent_dir,'/theta_vs_diameter.png'));

f4 = figure(4);
plot(angle,height,'-r',theta,Z/inchestomm);
xlabel('theta(rad)');ylabel('height(in)');set(findall(f4, 'Type', 'Line'),'LineWidth',4);set(gca,'FontSize',15);
saveas(f4,strcat(parent_dir,'/theta_vs_height.png'));
close all;
